...
| LivePortrait Real Run | Feature Flag |
...
V0.6 Sprint 23 adds `realRun` behind an explicit Runtime Host feature flag.

Real execution is disabled by default.

Enable it only when you have prepared your own local LivePortrait runtime:
...
Use another port if `8787` is busy:

```bash
RUNTIME_HOST_PORT=8788 npm run runtime:host
```

The host still rejects raw commands. `realRun` executes only the commandPlan generated by Runtime Host from validated LivePortrait input and runtime config.

V0.6 Sprint 23 新增 feature flag 保护下的 `realRun`。默认关闭真实执行；只有用户显式设置 `RUNTIME_ENABLE_REAL_EXECUTION=true` 后，本地 Runtime Host 才允许执行自己生成的 commandPlan。
...
- Sprint 24: Runtime Output Video Import into Asset System
